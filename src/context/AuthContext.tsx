...
import { mockDirectors, mockProducers } from '../data/mockData';
  ...
  const login = async (email: string, role: 'director' | 'producer', password: string): Promise<User> => {
    try {
      ...
      if (!isBackendAvailable) {
        throw new Error('Backend server is not available. Please ensure the server is running at http://localhost:3000');
      }
      ...
      // Ensure the user data includes the role
      const userData = {
        ...data,
